...
from case_based_modules.retrieve.bm25 import BM25
...
CF = 2 #number of central facts to consider for the final inference
...
facts_retriever.fit(corpus, question_train, ids, q_ids)
cases_retriever.fit(corpus, question_train, ids, q_ids)
...
for q_id, exp in tqdm(eb_dataset.items()):
    ...
    for choice in exp["_choices"]:
        ...
        #get hypothesis and entities (concepts)
        hypothesis = question + " " + candidate 
        ...
        # RETRIEVE
        relevance_scores = RS.compute(lemmatized_hypothesis, K)
        explanatory_power_scores = PW.compute(q_id, lemmatized_hypothesis, Q, QK)
